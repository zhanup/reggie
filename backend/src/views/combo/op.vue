          ...
          <div class="items">
            <div v-for="(item, ind) in checkedList" :key="ind" class="item">
              ...
              <span class="del" @click="delCheck(ind)">
                <img
                  :src="require('@/assets/images/icons/btn_clean@2x.png')"
                  alt=""
                />
              </span>
            </div>
...
import { getCategoryList, queryDishList } from '@/api/food'
    ...
    async init() {
      querySetmealById(this.id).then((res) => {
        if (String(res.code) === '1') {
          ...
          this.imageUrl = `/api/common/download?name=${res.data.image}`
